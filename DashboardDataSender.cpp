		...
		dash.AddCluster(); // target Info (2 elements)
		{
			dash.AddArray(); // targets
			{
				for (unsigned i = 0; i < targets.size(); i++)
				{
					...
					{
						dash.AddDouble(targets[i].score); // target score
						dash.AddCluster(); // Circle Description (5 elements)
						{
							dash.AddCluster(); // Position (2 elements)
							{
								dash.AddFloat((float) (targets[i].xPos / targets[i].width)); // X
								dash.AddFloat((float) targets[i].yPos); // Y
							}
							dash.FinalizeCluster();

							dash.AddDouble(targets[i].width); // Angle
							dash.AddDouble(targets[i].height); // Major Radius
							dash.AddDouble(targets[i].xPos); // Minor Radius
							dash.AddDouble(targets[i].yPos); // Raw score
						}
